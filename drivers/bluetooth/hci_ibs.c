...
static inline int ibs_check_data_len(struct ibs_struct *ibs, int len, struct hci_dev *hdev)
{
	...
	while (count) {
		if (ibs->rx_count) {
			...
			case HCI_IBS_W4_EVENT_HDR:
				...
				ibs_check_data_len(ibs, eh->plen, hu->hdev);
				continue;
			...
			case HCI_IBS_W4_ACL_HDR:
				...
				ibs_check_data_len(ibs, dlen, hu->hdev);
				continue;
			...
			case HCI_IBS_W4_SCO_HDR:
				...
				ibs_check_data_len(ibs, sh->dlen, hu->hdev);
				continue;
			}
		}
